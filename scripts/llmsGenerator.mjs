...
function getI18nConfig(config) {
  const i18n = config.internationalization || config.i18n;
  if (!i18n) return null;

  const defaultLocale = i18n.defaultLocale || i18n.default_language;
  const locales = Array.isArray(i18n.locales)
    ? i18n.locales.map((l) => (typeof l === "string" ? l : l.value))
    : [];
...
function isNonDefaultLocale(urlPath, i18n) {
  if (!i18n) return false;
  const firstSegment = urlPath.split("/").filter(Boolean)[0];
  if (!firstSegment) return false;
  return (
    i18n.locales.includes(firstSegment) && firstSegment !== i18n.defaultLocale
  );
}
...
function stripDefaultLocale(urlPath, defaultLocale) {
  if (!defaultLocale) return urlPath;
...
function extractReactFlightContent(html) {
  // In React Flight streaming HTML, quotes inside JSON are escaped as \"
  // Extract title from metadata component (has meta_title alongside title)
  ...
  if (!title) {
    ...
    if (titleMatch) return { title: titleMatch[1], description: "", content: "" };
  }
  ...
  // Extract content from React Flight chunks
  const chunks = [];
  const pushRegex = /self\.\w+\.push\((\[\d+,[\s\S]*?\])\)/g;
  let m;
  ...
  for (const chunk of chunks) {
    // Extract dangerouslySetInnerHTML content (main page HTML)
    const htmlMatches = chunk.matchAll(
    ...
    // Extract children strings (paragraphs, list items)
    const childMatches = chunk.matchAll(
      /\\"children\\":\\"((?:\\\\.|[^\\"\\\\])*)\\"/g,
    );
    for (const cm of childMatches) {
      const text = cm[1]
        ...
        .replace(/\\\\t/g, "\t")
        .replace(/\\\\\\\\/g, "\\");
      // Skip short strings, URLs, CSS classes, and nav/footer items
      if (
        text.length > 15 &&
        !text.startsWith("/") &&
        !text.includes("sr-only") &&
        !text.includes("aria-label") &&
        !text.includes("noopener") &&
  ...
  // If we collected raw HTML fragments, try to convert them
  if (content.includes("<") && content.includes(">")) {
    try {
      content = turndownService.turndown(content);
    } catch {
      // leave as-is if turndown fails
    }
  }
...
async function processHtml(html, llmsConfig) {
  ...
  // If normal parsing found nothing, try React Flight fallback
  if (!title && html.includes("self.__next_f")) {
    const flight = extractReactFlightContent(html);
    return {
      title: flight.title || title,
      description: flight.description || description,
      content: flight.content || content,
    };
  }
...
/**
 * Fallback: spawns `next start` on TEMP_PORT when standalone output isn't available.
 */
async function startNextStartServer() {
  const nextBin = path.join(process.cwd(), "node_modules", ".bin", "next");
  if (!fs.existsSync(nextBin)) {
    console.log("   ⚠️  next binary not found.");
    return null;
  }

  console.log(`   Spawning 'next start' on port ${TEMP_PORT}...`);

  const serverProcess = spawn(nextBin, ["start", "-p", String(TEMP_PORT)], {
    env: { ...process.env, PORT: String(TEMP_PORT), HOST: TEMP_HOST },
    stdio: ["ignore", "pipe", "pipe"],
    detached: false,
  });
  ...
  if (!ready) {
    console.log("   ⚠️  'next start' did not become ready in time.");
    serverProcess.kill();
    return null;
  }

  console.log(`   ✓ Next server ready at ${tempBaseUrl}`);
  return { process: serverProcess, baseUrl: tempBaseUrl };
}

/**
 * Starts a temp server: tries standalone first, falls back to `next start`.
 */
async function startAnyTempServer() {
  let handle = await startTempServer();
  if (!handle) {
    handle = await startNextStartServer();
  }
  return handle;
}
 ...
 */
function discoverSsrPageRoutes(srcDir, basePath, inputDir, defaultLocale = null) {
  if (!fs.existsSync(srcDir)) return [];

  const isAppRouter = srcDir.endsWith("app");
  const routes = [];

  // If App Router has [locale], scan inside it
  const localeDir = isAppRouter && defaultLocale
    ? path.join(srcDir, "[locale]")
    : null;
  const scanRoot = localeDir && fs.existsSync(localeDir) ? localeDir : srcDir;
  ...
  function scanDir(dir, urlPrefix) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        let normalizedRoute = routePath || "/";

        // Prefix with default locale when scanning inside [locale]
        if (localeDir && scanRoot === localeDir && defaultLocale) {
          normalizedRoute = normalizedRoute === "/"
            ? `/${defaultLocale}`
            : `/${defaultLocale}${normalizedRoute}`;
        }
  ...
  scanDir(scanRoot, basePath === "/" ? "" : basePath.replace(/\/$/, ""));
...
async function generateLlmsFiles() {
  ...
  const hasLocaleRoute =
    fs.existsSync(srcAppDir) &&
    fs.existsSync(path.join(srcAppDir, "[locale]"));

  const pages = [];
  const seenPaths = new Set();
  let tempServerHandle = null;

  // ── Strategy A: App Router with [locale] ────────────────────────────────
  // Static HTML files are React Flight streaming format (unparseable).
  // Discover all URL paths from build output, then fetch from a server.
  if (hasLocaleRoute) {
    console.log("🌍 App Router with [locale] detected. Using server fetching.\n");

    const defaultLocale = i18n?.defaultLocale || "en";
    console.log("🔍 Discovering pre-rendered routes...");
    const htmlFiles = await discoverHtmlFiles(inputDir);

    const allRoutes = htmlFiles
      .map((f) => fileToUrlPath(f, inputDir))
      .filter((r) => !isApiRoute(r) && !isNonDefaultLocale(r, i18n))
      .map((r) => stripDefaultLocale(r, defaultLocale))
      .filter((r) => {
    ...
    // Deduplicate after locale stripping
    const uniqueRoutes = [...new Set(allRoutes)];

    console.log(`   Found ${uniqueRoutes.length} default-locale route(s)`);
    ...
    if (uniqueRoutes.length > 0) {
      ...
      if (await isServerRunning(siteUrl)) {
        ...
        tempServerHandle = await startAnyTempServer();
        if (tempServerHandle) {
          fetchBase = tempServerHandle.baseUrl;
        } else {
          console.log(
            "   ⚠️  No server available. Locale pages will be skipped.\n" +
    ...
    // ── Strategy B: Standard static HTML processing ─────────────────────
    ...
    for (const file of htmlFiles) {
      try {
        ...
        if (isNonDefaultLocale(urlPath, i18n)) {
          console.log(`   ⤷ Skipping non-default locale: ${urlPath}`);
          continue;
        }
    ...
    const missingRoutes = ssrRoutes.filter((r) => {
      ...
      if (isNonDefaultLocale(r, i18n)) return false;
      const isExcluded = shouldExcludeRoute(r, llms.exclude);
      const isIncluded = shouldIncludeRoute(r, llms.include);
      return !(isExcluded && !isIncluded);
    });
