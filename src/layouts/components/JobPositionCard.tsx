...
const JobPositionCard = (
  ...
  const handleFetchJobPositions = async () => {
    try {
      ...
      setJobPositions(updateJobPositions.initialData);
    } catch (error) {
  ...
  console.log("Added Job Position: ", updateJobPositions);
  console.log("All Job Positions: ", initialData);
        ...
        {jobPositions &&
          jobPositions.map((data: JobPosition) => (
            ...
            >
              ...
              <Link
                key={data._id}
                className="bg-primary rounded-md p-2 text-white font-bold"
                href={{
